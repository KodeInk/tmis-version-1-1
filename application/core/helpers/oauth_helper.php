...
function callcontact($consumer_key, $consumer_secret, $guid, $access_token, $access_token_secret, $usePost=false, $passOAuthInHeader=true)
{
  ...
  if (!empty($response)) {
    list($info, $header, $body) = $response;
    #Now get the actual emails
	if ($body) {
      logit("callcontact:INFO:response:");
      $contact = json_decode(json_pretty_print($body), true);
	  return extract_contacts_list($contact);
    }
    $retarr = $response;
  }
...
function extract_contacts_list($contactObj)
{
	$contacts = array();
		
	if(!empty($contactObj['contacts']['contact']))
	{
		foreach($contactObj['contacts']['contact'] AS $key=>$aContact)
		{
			if(!empty($aContact['fields']))
			{
				foreach($aContact['fields'] AS $contactValues)
				{
					if(array_key_exists('type', $contactValues) && array_key_exists('value', $contactValues))
					{
						$contacts[$key][$contactValues['type']] = $contactValues['value'];
					}
				}
			}
		}
